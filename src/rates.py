...
from lib.workflow import Workflow, ICON_ERROR, ICON_WARNING, ICON_INFO
...
def validate_currencies(queries, query, src, dst, currencies):
    ...
    if src not in currencies or dst not in currencies:
        show_autocomplete(queries, query, currencies)
        return False
    else:
        return True


def search_rate(src, dst):
    """
    ...
    if Decimal('0.0001').compare(rate) == 0:
        ...
        inverted_rate = search_rate(dst, src)
...
def process_conversion(queries, query, src, dst, val, currencies):
    """
    ...
    ####################################################################################################
    if not validate_currencies(queries, query, src, dst, currencies):
        return 100

    rate = search_rate(src, dst)
...
def show_autocomplete(queries, query, currencies):
    """
...
def handle_set_default_currency(args, currencies):
...
def handle_get_default_currency():
...
def handle_set_default_divisor(args):
...
def handle_get_default_divisor():
...
def main(wf):
    ...
    if args.default_currency:
        return handle_set_default_currency(args, currencies)

    ############################################################################################
    # Get the default currency
    ############################################################################################
    if args.get_default_currency:
        return handle_get_default_currency()

    ############################################################################################
    # Update the default divisor
    ############################################################################################
    if args.default_divisor:
        return handle_set_default_divisor(args)

    ############################################################################################
    # Get the default divisor
    ############################################################################################
    if args.get_default_divisor:
        return handle_get_default_divisor()
    ...
    for query in queries:
        if query and len(query) == 1:
            ...
            ret = process_conversion(queries, query, currency_src, currency_dst, None, currencies)
            returns.append(ret)
            ...
            ret = process_conversion(queries, query, src, dst, val, currencies)
            returns.append(ret)
            ...
            ret = process_conversion(queries, query, currency_src, currency_dst, val, currencies)
            returns.append(ret)
        else:
            show_autocomplete(queries, query, currencies)
            returns.append(100)
